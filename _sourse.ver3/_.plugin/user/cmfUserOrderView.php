...
class cmfUserOrderView extends cmfMainAjax {

	function __construct($orderId, $formUrl=null, $name=null, $func=null) {
        $this->set('$orderId', $orderId);
		if(!$name)		$name = 'orderView';
		if(!$formUrl)	$formUrl = cmfControllerUrl .'/user/orderView/?orderId='. $orderId;
	...
	static public function isView($orderId) {
        return cmfSession::is('$orderId'. $orderId);
	}
	static public function setView($orderId) {
        return cmfSession::set('$orderId'. $orderId, 1);
	}
	...
	public function run1() {
        ...
        $order = cmfRegister::getSql()->placeholder("SELECT EMS, data, deliveryDesc FROM ?t WHERE id=? AND status ?@ AND `delete`='no'", db_basket, $orderId, array_keys($status))
								->fetchAssoc();
		if(!$order) {
		    exit;
		}
		...
		if($email2!==$email) {
		    $this->getForm()->setError('email', 'Неправильный адрес');
			$this->runEnd(true);
		    exit;
		}
        if(!empty($order['deliveryDesc']))
            $deliveryDesc = cmfString::unserialize($order['deliveryDesc']);
        ...
        ?>
                                    ...
                                    <? if(isset($deliveryDesc)) { ?>
                                        ...
                                        <div class="cmfHide"><div id="contentDelivery" class="item-block">
        ...
        <?
        cmfAjax::get()->html('.confidentialData', ob_get_clean());
