...
class cmfUserModel extends cmfDriverUser {
    ...
    static protected function getDataParam($id) {
        $row = cmfRegister::getSql()->placeholder("SELECT data, ?fields FROM ?t WHERE id=?", self::getFieldsParam(), db_user_data, $id)
                                        ->fetchAssoc();
        if($row) {
	...
	static public function saveParam($send, $id) {
		...
		cmfRegister::getSql()->add(db_user_data, $save, array('id'=>$id, 'AND', '1'));
	}
