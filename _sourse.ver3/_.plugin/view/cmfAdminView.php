...
class cmfAdminView {
	...
	static public function url2($url, $name) {
		return self::url2('<b>'. ($name ? $name : '__'). '</b>');
	}
	...
	static function buttonType1($onclick, $name) {
		...
		return '<li class="item_botton" id="'. $id .'"><a onclick="'. $onclick .'"><span class="botton_and_radio_container_right_text_botton">'. $name .'</span></a></li>';
	}
	static function buttonType2($url, $name) {
		return '<li class="item_botton"><a href="'. $url .'"><span class="botton_and_radio_container_right_text_botton">'. $name .'</span></a></li>';
	}
	static function buttonType3($url, $name) {//cmfAdminUrl
		return '<span class="button"><a href="'. $url .'">'. self::view($name) .'</a></span>';
	}
	static function buttonType4($url, $name) {//cmfAdminUrl
	...
	static function onclickType1($onclick, $name) {//cmfAdminOnclickA
		return '<span class="button"><a class="button" onclick="'. $onclick .'">'. $name .'</a></span>';
	}
	static function onclickType2($onclick, $name) {//cmfAdminOnclick_ib
		return '<span onclick="'. $onclick .'" class="button link"><i><b>' .$name. '</b></i></span>';
	}
	static function onclickType3($onclick, $name) {//cmfAdminOnclick_b
		return '<div class="bt" onclick="'. $onclick .'"><i><b><a title="' .$name. '">' .$name. '</a></b></i></div>';
	}
	static function onclickType4($onclick, $name) {//cmfAdminOnclick_bl
		return '<div class="bt_l bt" onclick="'. $onclick .'"><i><b><a title="' .$name. '">' .$name. '</a></b></i></div>';
	}
