...
//REPLACE `s03_brokerage_yachts` SELECT `id`, 0, `type`, `data`, 'no', `url`, `title_h1`, `text`, '', `technical`, year, `long`, people,CEILING(`price`), '',`name_foto`, `main_foto`, main_foto_thumb_best, `main_foto_thumb`, `title`, `keywords`, `description` FROM 2011_01_old.brokerage
/*$sql = cmfRegister::getSql();
$res = $sql->placeholder("SELECT id, image_main, image_small, image_best FROM ?t WHERE visible='no' LIMIT 0, 3000", db_brokerage_yachts)
            ->fetchAssocAll('id');
$path = cmfWWW . cmfPathBrokerageYachts;
foreach($res as $k=>$row) {
    ...
    $sql->add(db_brokerage_yachts, $send, $k);
    //sleep(1);
}
*/
...
UPDATE `s03_brokerage_yachts` SET `param`='', `visible`='no';
UPDATE `s03_brokerage_yachts` s SET `param`='', `notice`=(SELECT text FROM 2011_01_old.yacht i WHERE i.id=s.id), `paramText`=(SELECT technical FROM 2011_01_old.yacht i WHERE i.id=s.id), `visible`='no'
*/
/*$sql = cmfRegister::getSql();
...
$res = $sql->placeholder("SELECT id, paramText, year, `long`, people FROM ?t WHERE visible='no' LIMIT 0, 5000", db_brokerage_yachts)
            ->fetchAssocAll('id');
foreach($res as $k=>$row) {
    ...
    foreach($paramText as $line) if(!empty($line)){
        ...
        if(!isset($_param[$n])) {
            if(isset($_search[$n])) {
                ...
                pre('Нет параметра', $paramText, $k, $row['paramText'], $n, $v);
                die();
            }
        }
        $res = $_param[$n]['value'];
...
die();*/
/*
UPDATE  2011_01.s03_brokerage_yachts a SET a.`visible`='no',
a.`image_main`=(SELECT main_foto FROM 2011_01_old. brokerage o WHERE o.id=a.id),
a.`image_small`=(SELECT main_foto_thumb FROM 2011_01_old. brokerage o WHERE o.id=a.id),
a.`image_best`=(SELECT main_foto_thumb_best FROM 2011_01_old. brokerage o WHERE o.id=a.id)
*//*
$sql = cmfRegister::getSql();
$res = $sql->placeholder("SELECT id, image_main, image_small, image_best FROM ?t WHERE visible='no'", db_brokerage_yachts)
            ->fetchAssocAll('id');
$path = cmfWWW . cmfPathBrokerageYachts;
foreach($res as $k=>$row) {
    ...
    $file3 = $path . $row['image_best'];
    //file_put_contents($file2, file_get_contents($main1));
    //cmfFile::copy($file2, $file3);

    //cmfImage::resize($file2, yachtsSmallWidth, yachtsSmallHeight);
    //cmfImage::resize($file3, yachtsBestWidth, yachtsBestHeight);

    $image = array('image'=>array('image_main'=>$row['image_main'], 'image_small'=>$row['image_small'], 'image_best'=>$row['image_best']));
    $send = array();
    $send['visible'] = 'yes';
    $send['image'] = serialize($image);
    $sql->add(db_brokerage_yachts, $send, $k);
...
$main_list = $page->load('brokerage_yachts_list_controller');
