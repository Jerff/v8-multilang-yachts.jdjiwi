...
abstract class driver_db_list extends driver_db_edit {
	...
	protected function runListQuery($id=null, $offset=null, $limit=null) {
		if(is_null($id)) {
			return $this->getSql()->placeholder("SELECT SQL_CALC_FOUND_ROWS ?f FROM ?t WHERE ?w ORDER BY ?o LIMIT ?i, ?i ", $this->getFields(), $this->getTable(), $this->getWhereFilter(), $this->getSort(), $offset, $limit);
		}
		else {
			if(empty($id)) return false;
			return $this->getSql()->placeholder("SELECT ?f FROM ?t WHERE ?w", $this->getFields(), $this->getTable(), $this->getWhereId($id)); // AND ?w, $this->filter()
		}
	}
	...
	public function getTotal() {
		return $this->getSql()->getFoundRows();
	}
...
/*	public function updatePos($pos) {
	...
	private function move($id, $type) {
		...
		if(!$row = $sql->placeholder("SELECT `{$fields}` FROM ?t WHERE id=?", $table, $id)
							->fetchRow()) {
			return false;
		}
		list($pos1) = $row;
		...
		if($join) {
			$row = $sql->placeholder("SELECT a.id, a.`{$fields}` FROM ?t a, ?t b WHERE ?w AND a.`{$fields}`{$sort1}b.`{$fields}` ORDER BY a.`{$fields}` {$sort2} LIMIT 0, 1", $table, $table, $where)
							->fetchRow();
		} else {
			$row = $sql->placeholder("SELECT a.id, a.`{$fields}` FROM ?t a, ?t b WHERE a.id!=b.id AND b.id=? AND a.`{$fields}`{$sort1}b.`{$fields}` ORDER BY a.`{$fields}` {$sort2} LIMIT 0, 1", $table, $table, $id)
							->fetchRow();
		}
		if(!$row) return false;
		list($id2, $pos2) = $row;
