...
abstract class driver_db_edit_product_action extends driver_db_edit {
		...
		if($send) {
			...
			foreach($this->getActionKey() as $key) {
				...
				if($value==='yes') {
					...
					$this->getSql()->del($this->getTable(), array($product=>$id, 'AND', $action=>$key));
					$this->updateDeleteData($id, $key);
				}
			}

			$this->setUpdateData($action);
		}
	}
	...
	public function runData() {
		$res = $this->getSql()->placeholder("SELECT * FROM ?t WHERE ?w", $this->getTable(), $this->getWhere());
		$data = array();
	...
	public function deleteProduct($id) {
		$product = $this->product();
		$this->getSql()->del($this->getTable(), array($product=>$id));
		$this->setUpdateData($id);
	}

	public function deleteAction($id) {
		$action = $this->action();
		$this->getSql()->del($this->getTable(), array($action=>$id));
		$this->setUpdateData($id);
	}
