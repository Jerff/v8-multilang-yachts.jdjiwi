...
class _pages_admin_list_db extends driver_db_list_one {
	...
	public function getSelectGroup(&$select) {
		$menu = $this->getSql()->placeholder("SELECT p.`id`, p.`parent`, p.`name`, p.`type`, count(s.id) AS count FROM ?t p LEFT JOIN ?t s ON(p.id=s.parent AND s.type='pages') WHERE p.visible='yes' AND p.type IN('tree', 'list') GROUP BY p.id ORDER BY ?o:p", $this->getTable(), $this->getTable(), $this->getSort())
