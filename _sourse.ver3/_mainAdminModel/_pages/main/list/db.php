...
class _pages_main_list_db extends driver_db_list_one {
	...
	public function getSelectTree(&$element, $index=false, $isTitle=true) {
		...
		$res = $sql->placeholder("SELECT id, parent, name FROM ?t WHERE visible='yes' AND type IN('tree', 'list') ORDER BY ?o", $this->getTable(), $this->getSort());
		$menu = array();
		...
		if($isTitle) {
			$_title=array();
			$res = $sql->placeholder("SELECT uri,CONCAT(IF(LENGTH(title),'t',0),IF(LENGTH(keywords),'k',0),IF(LENGTH(description),'d',0)) FROM ?t WHERE lang=?", db_seo_title, cmfLang::getId());
			while($row=$res->fetchRow())
				$_title[$row[0]] = '['. $row[1] .'] ';
			$res->free();
		}

		$res = $sql->placeholder("SELECT c.id, c.parent, c.name, c.small_name  FROM ?t c WHERE c.parent ?@ AND ((c.type='pages' AND c.url!='') OR (c.type='pagesSystem' AND c.small_name='/404/')) AND c.visible='yes' ORDER BY c.pos", $this->getTable(), $id);
		$pages = array();
	...
	public function getPagesVariables($id) {
		return $this->getSql()->placeholder("SELECT variables FROM ?t WHERE small_name=? AND visible='yes' AND type='pages'", db_pages_main, $id)
								->fetchRow(0);
	}
