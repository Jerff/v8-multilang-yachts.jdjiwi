...
class view_gallery extends view_list {
 	...
 	static public function posMove($jsModul, $name, $key, $pos) {
        $html = ' ';
 		if($key) {
			$cmfBaseUrl = cmfBaseUrl;
        	$html .=<<<HTML
		<table>
		<tr>
		<td rowspan="3" class="pos">
			<span id="{$name}posView{$key}">{$pos}</span>
		    <span id="{$name}posText{$key}" class="cmfHide">
		    <input id="{$name}pos{$key}" type="text" value="{$pos}" size="3"><input id="{$name}posOld{$key}" type="hidden" value="{$pos}">
		    </span></td>
		    <td><a href="javascript:void(0);" onclick="{$jsModul}.move1Ajax('{$key}');"><img src="{$cmfBaseUrl}/sourseImage/admin/pos_up.gif" class="img"></a></td>
		</tr>
		<tr>
		    <td height="11px"><a href="javascript:void(0);" onclick="{$jsModul}.moveMoveAjax(this, '{$name}', '{$key}');"><img src="{$cmfBaseUrl}/sourseImage/admin/pos_center.gif" class="img"></a></td>
		</tr>
		<tr>
		    <td height="9px"><a href="javascript:void(0);" onclick="{$jsModul}.move2Ajax('{$key}');"><img src="{$cmfBaseUrl}/sourseImage/admin/pos_down.gif" class="img"></a></td>
		</tr>
		</table>
HTML;
 		}
	...
	static public function preview(&$modul, $jsModul, &$data) {
	    if(!$modul->getId() or !$data->get('image_small')) return;
	    list($width, $height) = $modul->getSize();
	    $real = cmfConfig::read('gallery', 'real');
	    $view = self::width;
?>
...
<div class="container gallery">
  <div style="float: left; width: 50%;">
    ...
    <div class="frame" style="margin: 0 0.3em; width: <?=$view ?>px;">
      <img id="galleryPhotoId" src="<?=cmfBaseImg . $modul->getPath() . $data->image_main ?>" style="max-width: 100%"/>
    </div>
  </div>
  ...
  <div style="float: left; width: 50%;">
    ...
    <div class="frame" style="margin: 0 1em; width: <?=$width ?>px;">
      <div id="galleryPreviewId" style="width: <?=$width ?>px;">
        <img src="<?=cmfBaseImg . $modul->getPath() . $data->image_small .'?'. time() ?>" style="max-width: 100%"/>
      </div>
    </div>
    <br />
    <table style="margin-top: 1em;" class="cmfHide">
      <thead>
        <tr>
