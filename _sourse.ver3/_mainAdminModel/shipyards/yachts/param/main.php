...
/*
UPDATE `s03_shipyards_yachts` SET `visible`='no';
*/
/*$sql = cmfRegister::getSql();
$_view = $sql->placeholder("SELECT id, `type` FROM ?t", db_param)
                   ->fetchRowAll(0, 1);
pre($_view);
$res = $sql->placeholder("SELECT id, param FROM ?t WHERE visible='no'", db_shipyards_yachts)
            ->fetchAssocAll();
foreach($res as $row) {
    foreach(cmfString::unserialize($row['param']) as $k=>$v) {
        switch(get($_view, $k)) {
            ...
            case 'radio':
            case 'select':
                $sql->replace(db_param_select, array('group'=>'shipyards', 'id'=>$row['id'], 'param'=>$k, 'value'=>$v));
                break;
        }
    }
    $sql->add(db_shipyards_yachts, array('visible'=>'yes'), $row['id']);
}
die();*/
...
$main_edit = $page->load('shipyards_yachts_param_controller');
