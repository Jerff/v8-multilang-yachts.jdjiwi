...
/*//REPLACE `s03_shipyards_yachts_foto` SELECT `id`, `id_yacht`, 0, 'no', 'no', '', '', name, thumb FROM 2011_01_old.yacht_foto
$sql = cmfRegister::getSql();
$res = $sql->placeholder("SELECT id, yachts, image_main, image_small FROM ?t WHERE visible='no' ORDER BY id LIMIT 0, 8000", db_shipyards_yachts_foto)
            ->fetchAssocAll('id');
$path = cmfWWW . cmfPathShipyardsYachtsFoto;
foreach($res as $k=>$row) {
    ...
    $send = array();
    $send['pos'] = 1+$sql->placeholder("SELECT max(pos) FROM ?t WHERE yachts=? ORDER BY id", db_shipyards_yachts_foto, $row['yachts'])
                        ->fetchRow(0);
    $send['visible'] = 'yes';
    $send['image'] = serialize($image);
    $send['image_main'] = $file1;
    $send['image_small'] = $file2;
    $sql->add(db_shipyards_yachts_foto, $send, $k);
    //sleep(1);
}*/


/*//UPDATE `s03_shipyards_yachts_foto` SET `pos`='0'
$sql = cmfRegister::getSql();
//$sql->placeholder("UPDATE ?t SET `pos`='0'", db_shipyards_yachts_foto);
$res = $sql->placeholder("SELECT id, yachts FROM ?t WHERE pos='0' ORDER BY id DESC LIMIT 0, 100000", db_shipyards_yachts_foto)
            ->fetchAssocAll('id');
foreach($res as $k=>$row) {
    $send = array();
    $send['pos'] = 1+$sql->placeholder("SELECT max(pos) FROM ?t WHERE yachts=? ORDER BY id ", db_shipyards_yachts_foto, $row['yachts'])
...
/*
UPDATE  2011_01.s03_shipyards_yachts_foto a SET a.`visible`='no',
a.`image_main`=(SELECT name FROM 2011_01_old. yacht_foto o WHERE o.id=a.id),
a.`image_small`=(SELECT thumb FROM 2011_01_old. yacht_foto o WHERE o.id=a.id)
*//*
$sql = cmfRegister::getSql();
$res = $sql->placeholder("SELECT id, image_main, image_small FROM ?t WHERE visible='no'", db_shipyards_yachts_foto)
            ->fetchAssocAll('id');
$path = cmfWWW .cmfPathShipyardsYachtsFoto;
foreach($res as $k=>$row) {
...
$main_edit = $page->load('shipyards_yachts_foto_edit_controller');
$this->assing('id', $main_edit->getId());

$main_multi = $page->load('shipyards_yachts_foto_multi_controller');
$main_list = $page->load('shipyards_yachts_foto_list_controller');
$page->run();
