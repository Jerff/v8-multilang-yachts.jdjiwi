...
/*//REPLACE `s03_shipyards_yachts` SELECT `id`, `shipyard`, `type`, `data`, 'no', `url`, `title_h1`, `text`, '', `technical`, CEILING(`price`), '',`name_foto`, `main_foto`, `main_foto_thumb`, `main_foto_thumb`, `title`, `keywords`, `description` FROM 2011_01_old.yacht
$sql = cmfRegister::getSql();
$res = $sql->placeholder("SELECT id, image_main, image_small, image_best FROM ?t WHERE visible='no' LIMIT 0, 3000", db_shipyards_yachts)
            ->fetchAssocAll('id');
$path = cmfWWW . cmfPathShipyardsYachts;
foreach($res as $k=>$row) {
    ...
    $sql->add(db_shipyards_yachts, $send, $k);
    sleep(1);
}*/
...
UPDATE `s03_shipyards_yachts` SET `param`='', `visible`='no';
UPDATE `s03_shipyards_yachts` s SET `param`='', `notice`=(SELECT text FROM 2011_01_old.yacht i WHERE i.id=s.id), `paramText`=(SELECT technical FROM 2011_01_old.yacht i WHERE i.id=s.id), `visible`='no'
*/
/*$sql = cmfRegister::getSql();
...
$res = $sql->placeholder("SELECT id, paramText FROM ?t WHERE visible='no' LIMIT 0, 5000", db_shipyards_yachts)
            ->fetchAssocAll('id');
foreach($res as $k=>$row) {
...
/*
UPDATE  2011_01.s03_shipyards_yachts a SET a.`visible`='no',
a.`image_main`=(SELECT main_foto FROM 2011_01_old.yacht o WHERE o.id=a.id),
a.`image_small`=(SELECT main_foto_thumb FROM 2011_01_old.yacht o WHERE o.id=a.id)
*//*
$sql = cmfRegister::getSql();
$res = $sql->placeholder("SELECT id, image_main, image_small FROM ?t WHERE visible='no'", db_shipyards_yachts)
            ->fetchAssocAll('id');
$path = cmfWWW . cmfPathShipyardsYachts;
foreach($res as $k=>$row) {
...
$main_list = $page->load('shipyards_yachts_list_controller');
$this->assing('filterShipyards', $main_list->filterShipyards());
$this->assing('filterType', $main_list->filterType());
$this->assing('filterCatalog', $main_list->filterCatalog());
$page->run();


$this->assing('shipyards', $main_list->getFilter('shipyards'));
