...
//REPLACE `s03_arenda_yachts_plan` SELECT `id`, `id_arenda_plan`, 0, 'no', 'no', '', '', name, thumb FROM 2011_01_old.arenda_plan_foto
/*$sql = cmfRegister::getSql();
$res = $sql->placeholder("SELECT id, yachts, image_main, image_small FROM ?t WHERE visible='no' ORDER BY id DESC LIMIT 0, 5000", db_arenda_yachts_plan)
            ->fetchAssocAll('id');
$path = cmfWWW . cmfPathArendaYachtsPlan;
foreach($res as $k=>$row) {
    ...
    $send = array();
    $send['pos'] = 1+$sql->placeholder("SELECT max(pos) FROM ?t WHERE yachts=? ORDER BY id ", db_arenda_yachts_plan, $row['yachts'])
                        ->fetchRow(0);
    $send['visible'] = 'yes';
    $send['image'] = serialize($image);
    $send['image_main'] = $file1;
    $send['image_small'] = $file2;
    $sql->add(db_arenda_yachts_plan, $send, $k);
    //sleep(1);
}
*/

/*
UPDATE  2011_01.s03_arenda_yachts_plan a SET a.`visible`='no',
a.`image_main`=(SELECT name FROM 2011_01_old. arenda_plan_foto o WHERE o.id=a.id),
a.`image_small`=(SELECT thumb FROM 2011_01_old. arenda_plan_foto o WHERE o.id=a.id)
*//*
$sql = cmfRegister::getSql();
$res = $sql->placeholder("SELECT id, image_main, image_small FROM ?t WHERE visible='no'", db_arenda_yachts_plan)
            ->fetchAssocAll('id');
...
$main_edit = $page->load('arenda_yachts_plan_edit_controller');
$this->assing('id', $main_edit->getId());

$main_multi = $page->load('arenda_yachts_plan_multi_controller');
$main_list = $page->load('arenda_yachts_plan_list_controller');
$page->run();
