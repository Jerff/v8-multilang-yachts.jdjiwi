...
class user_list_db extends driver_db_list {
	...
	protected function runListQuery($id=null, $offset=null, $limit=null) {
		if(is_null($id)) {
			return $this->getSql()->placeholder("SELECT SQL_CALC_FOUND_ROWS ?fields:u, ?fields:s FROM ?t u LEFT JOIN ?t s ON(u.id=s.id) WHERE ?w:u ORDER BY ?o:u LIMIT ?i, ?i ", $this->getFields(), $this->getFieldsSes(), $this->getTable(), db_user_ses, $this->getWhereFilter(), $this->getSort(), $offset, $limit);
		}
		else {
			if(empty($id)) return false;
			return $this->getSql()->placeholder("SELECT ?fields:u, ?fields:s FROM ?t u LEFT JOIN ?t s ON(u.id=s.id) WHERE ?w:u", $this->getFields(), $this->getFieldsSes(), $this->getTable(), db_user_ses, $this->getWhereId($id)); // AND ?w, $this->filter()
		}
	}
	...
	protected function getWhereFilter() {
		...
		if($name = $this->getFilter('name')) {
			$where[] = $this->getSql()->getQuery("name LIKE '%?s%'", urldecode($name));
			$where[] = 'AND';
		}

		if($email = $this->getFilter('email')) {
			$where[] = $this->getSql()->getQuery("email LIKE '%?s%'", urldecode($email));
			$where[] = 'AND';
		}
		$where[] = "(`admin` IS NULL)";
		return $where;
