    ...
    public function html($param, $style='') {
        ...
        if(is_null($param)) {
            ...
            foreach($this->getValueArray() as $key) {
                ...
                $str .= " <label><input $style type=\"radio\" name=\"$name\" value=\"". cmfString::specialchars($key) ."\" {$select} id=\"{$name}_{$key}\" />"
        ...
        $input = "<input $style type=\"radio\" name=\"$name\" value=\"". cmfString::specialchars($param) ."\" {$select} id=\"{$name}_{$key}\" />";

        if($param==='radio' or !$label) {
            return $input;
        } elseif($param==='label') {
...
class cmfFormRadioAll extends cmfFormRadio {
    public function jsUpdateValue() {
        $name = $this->getId();
        $content = cmfToJsString($this->html($name));
