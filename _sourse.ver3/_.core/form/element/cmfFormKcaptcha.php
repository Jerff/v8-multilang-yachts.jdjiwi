...
class cmfFormKcaptcha extends cmfFormText {
	...
	public function processing($data, $old, $upload) {
		$value2 = cmfSession::get('kcaptcha', $this->getKcaptchaName());
	...
	public function free() {
		cmfSession::del('kcaptcha', $this->getKcaptchaName());
	}
	...
	public function image($style='') {
		return '<img id="'. $this->getId() .'image" src="/library/kcaptcha/'. $this->getKcaptchaId(). '/" border="0" '. $style .'>';
	}
...
class cmfFormKcaptchaMain extends cmfFormKcaptcha {
