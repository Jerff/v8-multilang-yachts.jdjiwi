...
class cmfFormText extends cmfFormElement {
    ...
    public function html($param, $style = '') {
        return '<input type="' . $this->getType() . '" name="' . ($name = $this->getId()) . '" id="' . $name . '" value="' . cmfString::specialchars($this->getValue()) . '" ' . $style . ' />';
    }

}

class cmfFormTextMain extends cmfFormText {
...
class cmfFormTextSmall extends cmfFormText {

    public function htmlError($s = null, $s2 = null) {
        if ($this->isErrorView()) {
            return '<span class="formwrap_small">' . $s . $s2 . '</span>';
        } else {
            return $s . $s2;
        }
    }
...
class cmfFormTextRigthSmall extends cmfFormText {

    public function htmlError($s = null, $s2 = null) {
        if ($this->isErrorView()) {
            return '<span class="formwrap_small formwrap_figth_small">' . $s . $s2 . '</span>';
        } else {
            return $s . $s2;
        }
    }
...
class cmfFormTextInt extends cmfFormText {
    ...
    protected function init($o) {
        if (isset($o['min']))
            $this->setFilter('cmfFilterIntMin', $o['min']);
        if (isset($o['max']))
            $this->setFilter('cmfFilterIntMax', $o['max']);
        unset($o['min'], $o['max']);
...
class cmfFormDateMain extends cmfFormTextDate {
