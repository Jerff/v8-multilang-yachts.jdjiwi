...
class cmfFormImage extends cmfFormFile {
	...
	protected function upload($name, $file) {
		if($this->isUpload()) {
			$file = cmfFile::upload($this->getFolder(), $file);
		} else {
		...
		foreach($this->getSize() as $img=>$size) {
			...
			if($img==='main') {
				if(!empty($_POST['isReize'. $this->getId()])) {
				    cmfImage::resize($path. $file, $width, $height);
				}
				if($this->getWatermark()) {
                    cmfImage::watermark($path. $file);
				}
				$send['image'][$name .'_'. $img] = $file;
			} else {

				$newFile = $img .'/'. $file;
				if(cmfFile::copy($path . $file, $path . $newFile)) {
					cmfImage::resize($path . $newFile, $width, $height);
	...
	public function deleteFile(&$row, $key=-1) {
		...
		if(isset($value['image']))
			foreach($value['image'] as $key=>$file) {
				...
				cmfFile::unlink($this->getFolder() . $file);
			}
	}
	...
	public function copyFile(&$row, $name) {
		...
		if(isset($value['image']))
			foreach($value['image'] as $key=>$file)
			$value['image'][$key] = cmfFile::copy($this->getPath() . $this->getValue(), $this->getPath() . $this->getValue());
