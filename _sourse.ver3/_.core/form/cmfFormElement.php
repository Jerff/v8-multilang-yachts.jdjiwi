...
abstract class cmfFormElement {
    ...
    protected function init($o) {
        ...
        if (isset($o['length']))
            $this->setReform('cmfReformStrLen', $o['length']);
        if (isset($o['float']))
            $this->setReform('cmfReformFloat', false, true);
        if (isset($o['textFormat']))
            $this->setReform('cmfReformTextFormat', $o['textFormat']);
        if (isset($o['RusToEn']))
            $this->setReform('cmfReformRusToEn');
        if (isset($o['specialchars']))
            $this->setReform('cmfReformSpecialchars', false, $o['specialchars']);
        if (isset($o['uri'])) {
        ...
        if (isset($o['min']))
            $this->setFilter('cmfFilterLenMin', $o['min']);
        if (isset($o['max']))
            $this->setFilter('cmfFilterLenMax', $o['max']);
        if (isset($o['errrorHide']))
            $this->setErrorHidden();
        ...
        if (isset($o['empty']))
            $this->setFilter('cmfIsEmpty');
        if (isset($o['email']))
            $this->setFilter('cmfIsEmail');
        if (isset($o['phone']))
            $this->setFilter('cmfIsPhone');
        if (isset($o['url']))
            $this->setFilter('cmfIsUrl');
        if (isset($o['httpUrl']))
            $this->setFilter('cmfIsHttpUrl');
        if (isset($o['formatLengt']))
            $this->setFilter('cmfFilterTextLengt', $o['formatLengt']);
    ...
    public function setReform($name, $in = null, $out = null) {
        if (function_exists($name)) {
            ...
            if ($out === $in)
                ...
                $this->reform[$name] = array('in' => $in, 'out' => $out);
        }
    }
    ...
    protected function reformAll($value) {
        foreach ($this->getReform() as $reform => $opt) {
            if (is_array($opt))
                $value = $reform($value, $opt['out'], $opt['in']);
            else
    ...
    public function viewError() {
        return '<span class="formError" id="' . $this->getErrorId() . '"></span>';
    }

    public function htmlView($param, $style = '') {
        return $this->htmlError($this->html($param, $style), $this->htmlOld());
    }
    ...
    public function htmlOld() {
        return '<input type="hidden" name="' . ($name = $this->getOldId()) . '" id="' . $name . '" value="' . cmfString::specialchars($this->getValueOld()) . '" />';
    }

    // обновление формы
    public function jsUpdate() {
        $id = $this->getId();
        $error = cmfToJsString($this->getError());
        $js = '';
        if ($this->isErrorView()) {
    ...
    protected function jsUpdateValue() {
        $id = $this->getId();
        return "\ncmf.form.error.setValue('{$id}', '" . cmfToJsString($this->getValue()) . "');";
    }

    public function jsUpdateOld() {
        return "\ncmf.form.error.setValue('" . $this->getOldId() . "', '" . cmfToJsString($this->getValueOld()) . "');";
    }
