...
function cmfReformArrayPath($v, $in) {
    if ($in === 'serialize')
        return cmfString::arrayToPath($v);
    if (empty($v))
        return null;
    return cmfString::pathToArray($v);
}
...
function cmfReformUri($value, $opt = false, $opt2 = false) {
    if ($opt === false)
        return $value;
    $value = trim($value);
    $value = cmfString::translate($value);
    $value = trim(preg_replace(array('#\s#mS', '#[^a-z0-9\-_\.\=]#mSi', '#[_]{2,}#mSi'), '_', $value), '_');
    if ($opt2)
        $value = substr($value, 0, $opt2 - 1);
    return $value;
}
...
//$default_rfc_format='{d}-{m}-{Y} {H}:{M}:{S}';
    ...
    if (is_array($date)) {
        foreach ($date as $value)
            $this->reform_data($value, $input_format, $output_format);
        return null;
    }
...
//						'дек'=>'0','янв'=>'1','фев'=>'2','мар'=>'3','апр'=>'4','май'=>'5','июн'=>'6','июл'=>'7','авг'=>'8','сен'=>'9','окт'=>'10','ноя'=>'11','дек'=>'12');
    ...
    foreach ($date_value as $key => $value) {
        $value = trim($value);
        if (isset($local_date[$value]))
            $date_value[$key] = $local_date[$value];
    }
    ...
    $time = mktime($format_mktime['H'], $format_mktime['M'], $format_mktime['S'], $format_mktime['m'], $format_mktime['d'], $format_mktime['Y']);
...
function cmfReformSpecialchars($value, $opt = 1, $opt2 = 1) {
    ...
    if (in_array($opt, array(1, 'all', 'script')))
    ...
    foreach ($tag as $key2 => $value2) {
        if (!isset($tag_rules[$value2]['type']))
            continue;
    ...
    foreach ($tmp[1] as $key2 => $item_tag) {
        ...
        foreach ($tmp2[1] as $key3 => $value3) {
            if (isset($tag_rules[$item_tag]) and isset($tag_rules[$item_tag]['param'][$value3])) {
                ...
                if (is_string($func)) {
                    if ($func($tmp2[2][$key3]))
                        $tag_param[] = "$value3=" . trim($tmp2[2][$key3]);
                } else
                    $tag_param[] = "$value3=" . trim($tmp2[2][$key3]);
            }
        }
        if (!sizeof($tag_param))
            array_unshift($tag_param, $item_tag . ' ');
        else
            array_unshift($tag_param, $item_tag);
