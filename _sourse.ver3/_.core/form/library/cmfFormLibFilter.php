...
function cmfNotEmpty($v) {
	$r = !empty($v) or $v==='0';
	if(!$r) {
		cmfFormError::set(word("поле не должно быть пустым"));
...
function cmfIsEmail($v) {
	...
	if(!$r) {
		cmfFormError::set(word("в полe не почтовый адрес"));
...
function cmfFilterTextLengt($v, $o) {
	...
	if(!$r) {
		cmfFormError::set(word("неправильная длина поля"));
...
function cmfFilterLenMin($v, $m) {
	$len = mb_strlen((string)$v, cmfCharset);
	$r = (!$len or $len>=$m);
	if(!$r) {
		cmfFormError::set(str_replace('%n%', $m, word("поле не должно быть меньше (%m%) символов")));
	}
    return $r;
}
...
function cmfFilterLenMax($v, $m) {
	...
	$r = (!$len or $len<=$m);
	if(!$r) {
		cmfFormError::set(str_replace('%m%', $m, word("поле не должно быть больше (%m%) символов")));
	}
    return $r;
}
...
function cmfFilterIntMin($v, $m) {
	$r = ((int)$v >= (int)$m);
	if(!$r) {
		cmfFormError::set(str_replace('%m%', $m, word("поле меньше (%m%)")));
	}
    return $r;
}
...
function cmfFilterIntMax($v, $m) {
	$r = ($v <= $m);
	if(!$r) {
		cmfFormError::set(str_replace('%m%', $m, word("поле больше (%m%)")));
	}
    return $r;
}
