...
abstract class cmfSqlDriver {
	...
	public function query($query) {
		...
		if($res=$this->get()->query($query)) {
			if(cmfDebug::isSql()) {
				cmfDebug::addSql(trim($query), cmfGetMicrotime()-$t);
				if(cmfDebug::isExplain() and (stripos($query, 'SELECT')!==false)) {
					$exp = $this->get()->query('EXPLAIN '. $query)->fetchAssocAll();
					$query =  '<b>EXPLAIN</b> '. $query;
					foreach($exp as $r) {
						$query .= "\n". print_r($r, true);
					}
					cmfDebug::addExplain($query);
				}
			}
		} elseif(cmfDebug::isError()) {
			$error = $this->get()->errorInfo();
			$query = $error[2] .' 44 '. $query;
			cmfDebug::addError('<b>MYSQL ERROR</b> '. $this->get()->errorCode(), htmlspecialchars($query)) ;
		}
		return $res;
	}
	...
	function add($table, $data, $id=0) {
		if(is_array($id)) {
			if(sizeof($id)>1) return $this->add2($table, $data, $id);
			$key = key($id);
			$id = current($id);
		} else $key = 'id';
		if($id) {
	...
	private function _placeholder_run($m) {
		...
		switch($c) {
			...
			case 'fields':
				...
				$sep = '';
				while(list($k, $v) = each($a)) {
					if(is_string($k)) $str .= $sep. $t . $this->quoteParam($k) .' AS '. $v;
					else $str .= $sep. $t . $this->quoteParam($v);
					$sep = ', ';
				}
				return $str;
			...
			case 'o':
				while(list($k, $v) = each($a)) {
					...
					if(is_string($k)) $a[$k] = $t . $k .' '. $v;
					else $a[$k] = $t . $this->quoteParam($v);
				}
				return implode(', ', $a);
