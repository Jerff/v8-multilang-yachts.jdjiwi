...
class cmfCompilePhp {
	...
	private function file($file) {
		if(isset($this->file[$file])) return '';
		$this->file[$file] = true;

		foreach(cmfUpdateCompileConfig::includePath() as $path) {
            if(is_file($path . $file)) {
                $content = cmfConvertEncoding(file_get_contents($path . $file));
                break;
            }
		}
	...
	private function loadLibrary($m) {
		...
		switch($m[5]) {
			case 'LoadResponse':
				return "cmfLoad('ajax/cmfAjaxResponse');";

			case 'LoadAjax':
				return "cmfLoad('ajax/cmfAjax');";
	...
	private function comment3($m) {
		if(isset($m[3])) return $m[3][0] . $m[3][0];
		return $m[0];
	}
	private function comment4($m) {
		if(isset($m[3])) return $m[4];
		return $m[0];
	}
