...
class cmfControllerArticle extends cmfControllerDriver {
    const name = '/articles/item/';
    ...
    static public function updateSearch($where=null) {
        ...
        $res = $sql->placeholder("SELECT ?fields:a, ?fields:b FROM ?t a LEFT JOIN ?t b ON(b.id=a.id) WHERE ?w:a", $fields1, $fields2, db_article, db_article_lang, $where)
                                            ->fetchAssocAll();
        foreach($res as $row) {
            $send = array();
            ...
            $send['lang'] = $row['lang'];
            $send['page'] = self::name;
            $send['url'] = cmfGetLangUrl($row['lang'], '/articles/item/', array($row['uri']));
    ...
    static public function updateUri($where = null) {
        ...
        cmfRegister::getSql()->placeholder("
                REPLACE ?t SELECT ?, 'main', i.id, n.id, 0, CONCAT(i.isUri, '/', n.uri) FROM ?t n LEFT JOIN ?t i ON(i.pages='articles') WHERE ?w:n", db_content_url, self::name, db_article, db_menu_info, $where);
    }
