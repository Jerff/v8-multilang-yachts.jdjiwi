...
if ($yachtsList = cmfCache::get('main/Yacth')) {
    list($yachtsList, $typeList) = $yachtsList;
} else {
    ...
    if ($yachtsList) {
        ...
        if (!empty($typeList['arenda'])) {
            ...
            foreach ($res as $id => $row) {
                ...
                $typeList['arenda'][$id] = array(
                    ...
                    'image' => cmfImage::preview(cmfBaseOld . cmfPathArendaYachts . $row['image_small'], 250, 130, $row['name'], 'm-a', $row['id'], $row['update']),
        ...
        if (!empty($typeList['sale'])) {
            ...
            $res = $sql->placeholder("SELECT y.id, y.`update`, y.type, y.uri, y.image_small, y.param, y.price, y.currency, s.uri AS sUri FROM ?t y LEFT JOIN ?t s ON(s.id=y.shipyards) WHERE s.id=y.shipyards AND y.id ?@ AND y.visible='yes'", db_shipyards_yachts, db_shipyards, $typeList['sale'])
                    ->fetchAssocAll('id');
            ...
            foreach ($res as $id => $row) {
                ...
                $typeList['sale'][$id] = array(
                    ...
                    'image' => cmfImage::preview(cmfBaseOld . cmfPathShipyardsYachts . $row['image_small'], 250, 130, $row['name'], 'm-s', $row['id'], $row['update']),
                    'title' => htmlspecialchars($row['image_title']),
        ...
        if (!empty($typeList['brokerage'])) {
            ...
            $res = $sql->placeholder("SELECT y.id, y.`update`, y.type, y.uri, y.image_small, y.param, y.price, y.currency FROM ?t y WHERE y.id ?@ AND y.visible='yes'", db_brokerage_yachts, $typeList['brokerage'])
                    ->fetchAssocAll('id');
            ...
            foreach ($res as $id => $row) {
                ...
                $typeList['brokerage'][$id] = array(
                    ...
                    'image' => cmfImage::preview(cmfBaseOld . cmfPathBrokerageYachts . $row['image_small'], 250, 130, $row['name'], 'm-b', $row['id'], $row['update']),
                    'title' => htmlspecialchars($row['image_title']),
                    'price' => cmfPrice::view($row['price'], $row['currency']),
    ...
    cmfCache::set('main/Yacth', array($yachtsList, $typeList), 'shipyards');
}
