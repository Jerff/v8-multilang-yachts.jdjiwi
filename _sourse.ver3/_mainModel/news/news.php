...
$news = $sql->placeholder("SELECT id, image FROM ?t WHERE id=? AND visible='yes'", db_news, $newsId)
        ->fetchAssoc();
if (!$news)
    return 404;
$list = $sql->placeholder("SELECT lang, header, content, title, keywords, description FROM ?t WHERE id=?", db_news_lang, $newsId)
        ->fetchAssocAll('lang');
cmfLang::data($news, $list);
if (!empty($news['image'])) {
    $news['image'] = cmfBaseImg . cmfPathNews . $news['image'];
